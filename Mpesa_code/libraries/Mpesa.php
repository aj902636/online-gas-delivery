...
class Mpesa {
    ...
    public function __construct() {
        ...
        /* echo "<pre>"; 
          ...
          echo "<pre>"; 
          print_r($this->mpesa->c2b_validationURL);
          echo "</pre>"; die; */
    }
    ...
    public function C2B_REGISTER($status = 'Completed',$phone_number = null) {
        $url = $this->mpesa->c2b_regiterUrl;
        //echo $this->mpesa->c2b_validationURL;die;
        //pr($this->mpesa->c2b_validationUrl); die;
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_HTTPHEADER, array('Content-Type:application/json', 'Authorization:Bearer ' . $this->generatetoken()));
        $curl_post_data = array(
            'ShortCode' => trim($phone_number),
            'ResponseType' => $status,
            'ConfirmationURL' => $this->mpesa->c2b_confirmationURL,
            'ValidationURL' => $this->mpesa->c2b_validationURL
        );
        //pr($curl_post_data); die;
        $data_string = json_encode($curl_post_data);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_POSTFIELDS, $data_string);
...
//        die;
    ...
    public function B2C($CommandID, $amount, $remarks, $ocassion,$party_b = null) {
        //print_r($ocassion); die;
...
//            'PartyB' => $this->mpesa->test_msisdn,
        ...
        //echo "<pre>";
        //print_r($curl_post_data); echo "</pre>"; die;
        $data_string = json_encode($curl_post_data);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_POST, true);
        curl_setopt($curl, CURLOPT_POSTFIELDS, $data_string);
